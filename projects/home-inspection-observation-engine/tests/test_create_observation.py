...
from app.schemas import StructuredObservation, ObservationStatus, Photo
...
def test_get_observations_filtered_by_status(engine):
...
def test_create_observation_saves_photo_to_db(engine):
    with patch("app.api.create_basic_structured_observation") as mock_factory, \
         patch("app.api.analyze_image") as mock_analyze:
        mock_factory.return_value = StructuredObservation(
...
def test_get_observation_photo_returns_image(engine):
    with Session(engine) as session:
        photo = Photo(
...
def test_get_observation_photo_returns_404_for_wrong_observation(engine):
    with Session(engine) as session:
        photo = Photo(
...
def test_delete_observation_removes_observation_and_photos(engine):
    with Session(engine) as session:
