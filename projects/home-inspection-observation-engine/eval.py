...
def judge_free_text_field(field_name: str, expected: str, actual: str) -> dict:
    ...
    prompt = f"""You are evaluating a home inspection report field generated by an AI model.

Field: {field_name}
Expected: {expected}
Actual: {actual}

Score the actual output 1-10 based on semantic accuracy and quality:
1-2 = Completely wrong or missing key information
3-4 = Partially correct but missing important details
5-6 = Mostly correct with moderate differences
7-8 = Good, captures key information with slight variation
9-10 = Excellent, semantically equivalent to expected
...
def run_eval(change_note: str = "no note", enum_only: bool = False):
    ...
    if not enum_only:
        ...
        for r in results:
            ...
            for field, vals in r.get("judge_results", {}).items():
                print(f"  {vals['score']}/10 {field}: {vals['reason']}")
                judge_sum += vals["score"]
                judge_total += 1
            print()

        if judge_total > 0:
            print(f"Average LLM Judge Score: {judge_sum / judge_total:.1f}/10 across {judge_total} fields")

    save_run(change_note, results, correct, total, judge_sum, judge_total)

if __name__ == "__main__":
    args = [a for a in sys.argv[1:] if not a.startswith("--")]
    change_note = args[0] if args else "no note"
    enum_only = "--enum-only" in sys.argv
    run_eval(change_note, enum_only=enum_only)